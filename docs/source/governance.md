...
通过代币形式分发投票权利, 设置完成后, 治理的流程如下:

1. 提案

    提案, 就是在链上注册一个提案交易, 具体内容包括(target, data, value), 由proposer提出, 然后会在链上设置截止时间 (schedule), 开始让所有人投票.

1. 投票

    通过各种机制在链上收集用户投票, 当通过数达到某一个阈值后, 根据合约逻辑, 提案变为可执行.

2. 执行

    对于可执行的提案, 用户(或者指定executor)可以提交交易执行对应提案.

## `TimelockController.sol`

TimelockController的预期是作为一个executor, 接受通过DAO治理后的提案, 提案在TimelockController这里会锁定一段时间, 之后才可以开始执行.期间固定角色的用户可以撤销交易, 防止错误交易发生.
...
Governor的设计是作为DAO的核心骨架, 各个DAO的实现可以继承Governor去实现不同的计票, 提案逻辑等. Governor主要实现了提案的管理和执行逻辑.
